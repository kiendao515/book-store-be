...
public class FileUploadController {
    ...
    public BaseResponse<?> uploadFile(@RequestParam("file") MultipartFile multipartFile) throws IOException, BizException {
        ...
        if (isValidFile && allowedFileExtensions.contains(FilenameUtils.getExtension(multipartFile.getOriginalFilename()))) {
            FileDto uploadFileRes = fileService.uploadFile(multipartFile);
            String link = amazonProperty.getEndpointUrl() + "/" + uploadFileRes.getName();
            ImageDocument imageDocument = ImageDocument.builder()
                    .link(amazonProperty.getEndpointUrl() + "/" + uploadFileRes.getName())
                    .name(uploadFileRes.getName())
                    .type(uploadFileRes.getType())
                    .size(uploadFileRes.getSize())
                    .createdAt(ZonedDateTime.now())
                    .updatedAt(ZonedDateTime.now())
                    .build();
            ImageDocument image = imageRepository.save(imageDocument);
            APIResponse apiResponse = APIResponse.builder()
                    .message("file " + uploadFileRes + " uploaded successfully")
                    .url(link)
                    .id(image.getId())
                    .build();
